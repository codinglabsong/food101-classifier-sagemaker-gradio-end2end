...
def ensure_unpacked(path: str) -> str:
    ...
    if os.path.isdir(path):
        ...
        if len(entries) == 1 and entries[0].endswith((".tar", ".tar.gz", ".tgz")):
...
def main():
    ...
    train_tfms = transforms.Compose(
        [
            ...
            transforms.Normalize(
                [0.485, 0.456, 0.406], [0.229, 0.224, 0.225]  # ImageNet distribution
            ),
        ]
    )
    ...
    val_dl = DataLoader(
        val_ds, batch_size=cfg.batch_size, num_workers=cfg.num_workers, pin_memory=True
    )
    test_dl = DataLoader(
        test_ds, batch_size=cfg.batch_size, num_workers=cfg.num_workers, pin_memory=True
    )

    print(
        f"Data ready. len(train)={len(train_ds)}, len(val)={len(val_ds)}, len(test)={len(test_ds)}"
    )
    ...
    def epoch_loop(
        ...
        context = (
            autocast("cuda") if torch.cuda.is_available() else nullcontext()
        )  # increase GPU efficiency with autocast if available
        ...
        with torch.set_grad_enabled(is_train):
            for x, y in tqdm(loader, desc=phase):  # mini-batch loop
                ...
                (
                    optimizer.zero_grad(set_to_none=True) if is_train else None
                )  # saves a bit of GPU memory when setting to `none`
                ...
                if is_train:
                    if scaler:
                        ...
                        scaler.unscale_(
                            optimizer
                        )  # un-scale the gradients that live on the model (needed for gradient clipping)
                        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)

                        did_step = scaler.step(optimizer)
                        scaler.update()
                        if (
                            did_step and scheduler
                        ):  # must only call scheduler.step() if optimizer.step() actually happened. Otherwise the learning rate schedule will get out of sync when using GradScaler.
                            scheduler.step()
                    else:
                        loss.backward()  # back-prop
                        torch.nn.utils.clip_grad_norm_(
                            model.parameters(), max_norm=1.0
                        )  # gradient clipping for stability during training
        ...
        if phase in ["train", "val"]:
            # print statements
            print(
                f"{phase:5} | loss {epoch_loss:.4f} | acc {epoch_acc:.4f} | {dt:5.1f}s | {throughput:7.1f} samples/s"
            )
            if is_train and scaler:
            ...
            wandb.log(
                metrics
            )  # ensures logging at the same step as the last batch of that epoch
    ...
    if (
        ...
        print(
            f"GPU CC {torch.cuda.get_device_properties(DEVICE).major}.x detected - compiled model"
        )
